...
// Utility concepts
template< typename UP1, typename UP2 >
concept pointer_convertible =
   std::convertible_to<pointer_t<UP1>,pointer_t<UP2>> &&
   !std::is_array_v<element_t<UP1>>;

template< typename UP1, typename UP2 >
concept array_convertible =
   std::same_as<pointer_t<UP1>,element_t<UP1>*> &&
   std::same_as<pointer_t<UP2>,element_t<UP2>*> &&
   std::convertible_to<element_t<UP1>(*)[],element_t<UP2>(*)[]>;
...
class unique_ptr
{
 public:
   ...
   template< typename U, typename E >
      requires pointer_convertible< unique_ptr<U,E>, unique_ptr<T,D> >  // No constraint for 'E'
...
class unique_ptr<T[],D>
{
 public:
   ...
   template< typename U, typename E >
      requires array_convertible< unique_ptr<U,E>, unique_ptr<T,D> >  // No constraint for 'E'
   unique_ptr( unique_ptr<U[],E>&& u ) noexcept;
...
int main()
{
   // unique_ptr for a single Widget
   {
      ...
      unique_ptr<const WidgetBase> c( std::move( a ) );
      std::cout << " a has been moved to c (c=" << c->getInt() << ")\n";

      c = std::move( b );
      std::cout << " b has been moved to c (c=" << c->getInt() << ")\n";

      //unique_ptr<Widget> d( std::move(b) );  // Compilation error!!

      //unique_ptr<Widget> d{};
      //d = std::move(b);  // Compilation error!!!
   }
   ...
   // unique_ptr for an array of Widgets
   {
      ...
      //unique_ptr<WidgetBase[]> d( std::move( a ) );  // Compilation error!!!

      //unique_ptr<WidgetBase[]> d{};
      //d = std::move( a );  // Compilation error!!!
   }
