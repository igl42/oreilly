...
bool isYounger( Person const& person, int age )
{
   return person.age < age;
}

bool isOlder( Person const& person, int age )
{
   return person.age > age;
}
...
{
   return [age]( Person const& person ) { return isYounger(person,age); };
}

auto olderThan( int age )
{
   return [age]( Person const& person ) { return isOlder(person,age); };
}
...
template< typename Table >
void count_children( Table& table )
{
   ...
   size_t const count =
      std::count_if( std::begin(table), std::end(table),
         when_all( olderThan(6), youngerThan(18) ) );

   // C++20 ranges solution
   /*
   size_t const count =
      std::ranges::count_if( table, youngerThan(18) );
   */
