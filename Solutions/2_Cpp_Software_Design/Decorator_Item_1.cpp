...
{
 public:
   QuantityDiscounted( std::unique_ptr<Item>&& item, size_t count )
      : DecoratedItem{ std::move(item) }
      , count_{ count }
   {}
...
int main()
{
   ...
   std::unique_ptr<Item> item2(
      std::make_unique<Taxed>(
         std::make_unique<QuantityDiscounted>(
            std::make_unique<CppBook>( "Effective C++", Money{19} ),
            5 ),
         0.07 ) );

   std::unique_ptr<Item> item3(
      std::make_unique<ShoppingCart>( std::move(item1), std::move(item2) ) );
